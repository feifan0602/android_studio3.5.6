...
public class Options {
	...
	/** 图片加载配置 */
	public static DisplayImageOptions getLoginOptions() {

		DisplayImageOptions options = new DisplayImageOptions.Builder()
				// 设置图片在下载期间显示的图片
				.showImageOnLoading(R.drawable.sh_user_header_icon)
				// 设置图片Uri为空或是错误的时候显示的图片
				.showImageForEmptyUri(R.drawable.sh_user_header_icon)
				// // 设置图片加载/解码过程中错误时候显示的图片
				.showImageOnFail(R.drawable.sh_user_header_icon)
				// 设置下载的图片是否缓存在内存中
				.cacheInMemory(true)
				// 设置下载的图片是否缓存在SD卡中
				.cacheOnDisc(true).considerExifParams(true)
	...
	public static ImageLoaderConfiguration getImageConfig(Context context) {
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(
				...
				.imageDownloader(
						new BaseImageDownloader(context, 5 * 1000, 10 * 1000))
