...
class SeatGestureListener extends GestureDetector.SimpleOnGestureListener {
	...
	public boolean onScroll(MotionEvent paramMotionEvent1, MotionEvent paramMotionEvent2,
		...
		if (bool1) {
			...
			if (SeatView.getSeatViewRawDistance(mSatView) + mSatView.getMeasuredWidth() > SeatView
					.seatViewWidth(mSatView)) {
				// 滑到最右
				SeatView.i(mSatView, SeatView.seatViewWidth(mSatView) - mSatView.getMeasuredWidth());
				SeatView.a(mSatView,
						(float) (mSatView.getMeasuredWidth() - SeatView.seatViewWidth(mSatView)));
			}
		}
		...
		if (bool2) {
			...
			if (SeatView.t(mSatView) + mSatView.getMeasuredHeight() > SeatView
				...
				// 滑到底
				SeatView.j(mSatView,
						SeatView.seatViewHeight(mSatView) - mSatView.getMeasuredHeight());
				SeatView.b(mSatView,
						(float) (mSatView.getMeasuredHeight() - SeatView.seatViewHeight(mSatView)));
			}
		}
		...
		for (int i1 = 0; i1 < SeatView.list_SeatInfo(mSatView).size(); i1++) {
			...
			if (ch_s.getColumn() == i && ch_s.getRaw() == j) {
				...
				case SeatStatus.CHOOSE_OK:// 已选中
					...
					break;
				case SeatStatus.CHOOSE_UN:// 未选中
					if (NewOnlinSeatActivity.selectSeatlist.size() < SeatView.getMaxSeatCount()) {
