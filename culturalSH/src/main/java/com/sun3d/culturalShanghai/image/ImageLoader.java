...
public class ImageLoader {
	 ...
	 * 添加图片到缓存,有标签
	 */
	public static void addImageToLruCache(String path, String tag) {
		Bitmap bm = PicImgCompressUtil.get480Bitmap(path);
		if (bm != null) {
			addBitmapToLruCache(path + tag, bm);
		}
	}
	 ...
	 * 添加图片到缓存
	 */
	public static void addImageToLruCache(String path) {
		Bitmap bm = PicImgCompressUtil.get480Bitmap(path);
		if (bm != null) {
			addBitmapToLruCache(path, bm);
		}
	 ...
	 * @param bitmap
	 */
	private static void addBitmapToLruCache(String key, Bitmap bitmap) {
	...
	private Bitmap decodeSampledBitmapFromResource(String pathName, int reqWidth, int reqHeight) {
		...
		try {
			...
			return decodeSampledBitmapFromResource(pathName,reqWidth/2,reqHeight/2);
		}
		return bitmap;
	}
