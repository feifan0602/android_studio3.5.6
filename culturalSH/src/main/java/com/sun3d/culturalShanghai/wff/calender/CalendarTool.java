...
public class CalendarTool {
	...
	public List<DateEntity> getDateEntityList(int year, int month) {
		...
		if (previoursMonthDays == 7 || nextMonthDays >= 7) {
			dayOfNowCalendar = 35;
		}
		Log.i("ceshi", "看看所有的数据dayOfNowCalendar===" + dayOfNowCalendar
				+ "  days  == " + "previoursMonthDays  ==  "
				+ previoursMonthDays);
		nextMonthDays = dayOfNowCalendar - days - previoursMonthDays;
		/** 先添加前面不属于本月的 */
		for (int i = startDate, j = 0; i <= endDate && previoursMonthDays != 7; i++, j++) {
		...
		/*** 添加后面下一个月的 */
		for (int i = 1, j = selfDaysEndWeek + 1; i <= nextMonthDays
				&& nextMonthDays < 7; i++, j++) {
			mDateEntity = new DateEntity();
			mDateEntity.day = i;
			mDateEntity.isSelfMonthDate = false;
