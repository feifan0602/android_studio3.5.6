...
import com.sun3d.culturalShanghai.Util.PageUtil;
...
public class OutlineContainer extends FrameLayout implements Animatable {
	...
	private void init() {
		...
		mOutlinePaint.setStrokeWidth(PageUtil.dpToPx(getResources(), 2));
		mOutlinePaint.setColor(getResources().getColor(R.color.activity_bg_color));
		mOutlinePaint.setStyle(Style.STROKE);

		int padding = PageUtil.dpToPx(getResources(), 10);
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		int offset = PageUtil.dpToPx(getResources(), 5);
